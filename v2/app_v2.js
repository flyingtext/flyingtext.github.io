      ...
      <div className="card-body">
        ...
        <button className="form-control btn inline-block btn-primary" onClick={async (e) => {
          ...
          if(!document.getElementById("convert-herb-part").checked) {
            ...
            (SELECT * FROM (SELECT * FROM (SELECT 처방명, 출전출처, COUNT(*) as herbCount FROM prescription_structure WHERE (순수약재명 IN ("${processedHerbs.join('", "')}")) AND 약재명 != '' GROUP BY 처방명, 출전출처) as q1 LEFT OUTER JOIN (SELECT q0.처방명 as 처방명, q0.출전출처 as 출전출처, COUNT(*) as basicCount, group_concat(q0.순수약재명) as herbConst FROM (select * from prescription_structure order by 순수약재명) as q0 WHERE 순수약재명 != '' GROUP BY 처방명, 출전출처 ORDER BY 순수약재명) as q2 ON q1.처방명=q2.처방명 AND q1.출전출처=q2.출전출처 WHERE q1.herbCount >= ${leastMatchHerbNumber} AND q2.basicCount <= ${maxBasicHerbNumber}) as q3 GROUP BY q3.herbConst, q3.출전출처) as q4
            ...
            (SELECT * FROM (SELECT * FROM (SELECT 처방명, 출전출처, COUNT(*) as herbCount FROM prescription_structure WHERE (약재명 IN ("${processedHerbs.join('", "')}")) AND 약재명 != '' GROUP BY 처방명, 출전출처) as q1 LEFT OUTER JOIN (SELECT q0.처방명 as 처방명, q0.출전출처 as 출전출처, COUNT(*) as basicCount, group_concat(q0.약재명) as herbConst FROM (select * from prescription_structure order by 약재명) as q0 WHERE 약재명 != '' GROUP BY 처방명, 출전출처 ORDER BY 약재명) as q2 ON q1.처방명=q2.처방명 AND q1.출전출처=q2.출전출처 WHERE q1.herbCount >= ${leastMatchHerbNumber} AND q2.basicCount <= ${maxBasicHerbNumber}) as q3 GROUP BY q3.herbConst, q3.출전출처) as q4
